...
#[derive(Debug)]
pub enum Error {
    /// Cannot create a VCPU.
    VcpuFd(bhyve_api::Error),

    /// Cannot run the VCPUs.
    VcpuRun(bhyve_api::Error),
    ...
    SetSupportedCpusFailed(bhyve_api::Error),

    /// Unexpected VM run exit reason
    VcpuUnhandledExit,
    ...
    /// Failed to set VCPU MP state.
    VcpuSetMpState(bhyve_api::Error),

    /// Failed to get VCPU msrs.
    VcpuGetMsrs(bhyve_api::Error),

    /// Failed to set VCPU msrs.
    VcpuSetMsrs(bhyve_api::Error),

    /// Failed to get VCPU regs.
    VcpuGetRegs(bhyve_api::Error),

    /// Failed to set VCPU regs.
    VcpuSetRegs(bhyve_api::Error),

    /// Failed to get VCPU sregs.
    VcpuGetSregs(bhyve_api::Error),

    /// Failed to set VCPU sregs.
    VcpuSetSregs(bhyve_api::Error),

    /// Failed to get VCPU events.
    VcpuGetVcpuEvents(bhyve_api::Error),

    /// Failed to set VCPU events.
    VcpuSetVcpuEvents(bhyve_api::Error),

    /// Failed to get VCPU XSAVE.
    VcpuGetXsave(bhyve_api::Error),

    /// Failed to set VCPU XSAVE.
    VcpuSetXsave(bhyve_api::Error),

    /// Failed to get VCPU XCRS.
    VcpuGetXcrs(bhyve_api::Error),

    /// Failed to set VCPU XCRS.
    VcpuSetXcrs(bhyve_api::Error),
}
...
/// A wrapper around creating and using a bhyve-based VCPU.
pub struct Vcpu {
    fd: VirtualMachine,
    id: u8,
...
impl Vcpu {
    ...
    pub fn new(
        ...
        //fd.activate_vcpu(id as i32).map_err(Error::VcpuFd)?;
        // Bhyve doesn't have a separate VcpuFd type, so (for now) the VCPU
        // gets another copy of the VmFd.
        let vcpu_fd = VirtualMachine::new(&fd.name).map_err(Error::VcpuFd)?;
    ...
    pub fn run(&self) -> Result<bool> {
        match self.fd.run(self.id as i32) {
            Ok(run) => match run {
                VmExit::IoIn(addr, bytes) => {
                    let mut data: [u8; 4] = [0; 4];
                    let mut data_slice: &mut[u8] = match bytes {
                        1 => &mut data[0..1],
                        2 => &mut data[0..2],
                        4 => &mut data[0..],
                        _ => &mut [],
                    };
                    self.io_bus.read(u64::from(addr), data_slice);
                    Ok(true)
                }
                VmExit::IoOut(addr, bytes, value) => {
                    let data: [u8; 4] = value.to_le_bytes();
                    let data_slice: &[u8] = match bytes {
                        1 => &data[0..1],
                        2 => &data[0..2],
                        4 => &data[0..],
                        _ => &[],
                    };
                    if addr == DEBUG_IOPORT && bytes == 1 {
                        self.log_debug_ioport(data_slice[0]);
                    }
                    self.io_bus.write(u64::from(addr), data_slice);
                    Ok(true)
                }
...
//    fn bhyve_state(&self) -> Result<VcpuState> {
...
//    fn set_bhyve_state(&mut self, state: &VcpuState) -> Result<()> {
