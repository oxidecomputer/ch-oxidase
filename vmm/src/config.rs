...
impl NetConfig {
    ...
    \"tap=<if_name>,ip=<ip_addr>,mask=<net_mask>,iommu=on|off,\
...
mod tests {
    ...
    fn test_net_parsing() -> Result<()> {
        ...
        assert_eq!(
            NetConfig::parse(
                "tap=tap0,ip=192.168.100.1,mask=255.255.255.128"
            )?,
            NetConfig {
        ...
        assert_eq!(
            NetConfig::parse("socket=/tmp/socket")?,
            NetConfig {
        ...
        assert_eq!(
            NetConfig::parse("num_queues=4,queue_size=1024,iommu=on")?,
            NetConfig {
                num_queues: 4,
                queue_size: 1024,
                iommu: true,
