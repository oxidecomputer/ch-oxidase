...
pub enum Error {
    ...
    SetUserMemoryRegion(bhyve_api::Error),
...
impl MemoryManager {
    ...
    fn create_ram_region(
        ...
        Ok(Arc::new(match backing_file {
            ...
            None => {
                let mmap_flags = libc::MAP_NORESERVE | libc::MAP_ANONYMOUS
                    | if shared {
                        libc::MAP_SHARED
                    } else {
                        libc::MAP_PRIVATE
    ...
    pub fn create_userspace_mapping(
        ...
        self.fd.add_guest_memory(slot as i32, guest_phys_addr, userspace_addr, memory_size as usize, readonly)
    ...
    pub fn remove_userspace_mapping(
        ...
        self.fd.munmap_memseg(guest_phys_addr, memory_size as usize)
               .map_err(Error::SetUserMemoryRegion)?;
