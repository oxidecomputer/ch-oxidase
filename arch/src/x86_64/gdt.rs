...
mod tests {
    ...
    fn field_parse() {
        ...
        // 0xA09B
        // 'A'
        assert_eq!(0x1, get_g(gdt));
        assert_eq!(0x0, get_db(gdt));
        assert_eq!(0x1, get_l(gdt));
        assert_eq!(0x0, get_avl(gdt));
        // '9'
        assert_eq!(0x0, get_dpl(gdt));
        assert_eq!(0x1, get_s(gdt));
        // 'B'
        assert_eq!(0xB, get_type(gdt));
        // base and limit
        assert_eq!(0x100000, get_base(gdt));
        assert_eq!(0xffffffff, get_limit(gdt));
    }
}
